        ...
        <form method="POST" action="{{ route('tasks.update', $task) }}" class="task-form">
            ...
            <div class="form-group">
                ...
                <div class="search-select">
                    <input type="text" 
                           ...
                           autocomplete="off"
                           value="{{ old('organization_id') ? $organizations->find(old('organization_id'))->name : $task->organization->name }}">
                    <input type="hidden" name="organization_id" id="organization_id" value="{{ old('organization_id', $task->organization_id) }}">
            ...
            <div class="form-group">
                ...
                <div class="date-group" id="date_group">
                    <div class="date-input-wrapper">
                        <input type="text" name="completion_date" id="completion_date" class="form-control" 
                               value="{{ old('completion_date', $task->completion_date ? $task->completion_date->format('d.m.Y') : '') }}" 
                ...
                <div class="indefinite-option">
                    <input type="checkbox" id="indefinite_task" name="indefinite_task" value="1" 
                           {{ old('indefinite_task', !$task->completion_date) ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="priority" id="priority" class="form-control" required>
                    @for($i = 1; $i <= 10; $i++)
                        <option value="{{ $i }}" {{ old('priority', $task->priority) == $i ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <div class="assignees-list">
                    @foreach($users as $user)
                        @php
                            $isAssigned = old('assignees') 
                                ? in_array($user->id, old('assignees', [])) 
    ...
    <script>
        ...
        function initializeFlatpickr() {
            ...
            flatpickrInstance = flatpickr("#completion_date", {
                ...
                allowInput: true,
                clickOpens: true,
                defaultDate: "{{ old('completion_date', $task->completion_date ? $task->completion_date->format('d.m.Y') : '') }}",
                minDate: "today",
